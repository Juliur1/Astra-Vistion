...
class _WebsiteEditorDashboardPageState extends State<WebsiteEditorDashboard> {
  ...
  void _executeComponentInteraction(
    ...
    if (component['interactions'] == null) {
      print('No interactions found for ${component['type']}');
      return;
    }
    ...
    if (matchingInteractions.isEmpty) {
      print('No $interactionType interactions found for ${component['type']}');
      return;
    }
    ...
    for (var interaction in matchingInteractions) {
      print('Executing interaction: ${interaction['name']}');
  ...
  void _executeInteractionNodes(Map<String, dynamic> interaction) {
    if (interaction['nodeData'] == null || interaction['nodeData']['nodes'] == null) {
      print('No node data found for interaction: ${interaction['name']}');
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('No nodes configured for ${interaction['name']}. Please add nodes in the Interactions panel.'),
    ...
    if (nodes.isEmpty) {
      print('No nodes found in interaction: ${interaction['name']}');
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('No nodes configured for ${interaction['name']}. Please add nodes in the Interactions panel.'),
          duration: Duration(seconds: 3),
    ...
    if (onClickNodes.isEmpty) {
      print('No trigger nodes found in interaction: ${interaction['name']}');
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('No trigger nodes found. Please add an OnClick trigger node.'),
          duration: Duration(seconds: 3),
          backgroundColor: Colors.orange,
        ),
      );
          ...
          Center(
            child: LayoutBuilder(
              builder: (BuildContext context, BoxConstraints constraints) {
                // Calculate the size while maintaining aspect ratio - same as editing view
                ...
                return Container(
                  ...
                  child: Stack(
                    children: [
                      // Render all the components in preview mode with proper scaling
                      ..._screenComponents.map((component) {
                        // Calculate scaling factors to maintain proportions
                        final scaleX = width / _screenSettings.width;
                        final scaleY = height / _screenSettings.height;
                        final scale = min(scaleX, scaleY); // Use uniform scaling
  ...
  void _runAnimationByName(String animationName) {
    ...
    if (animationIndex != -1) {
      ...
      print('Animation execution completed for: $animationName');
    } else {
      print('Animation not found: $animationName');
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Animation "$animationName" not found! Please create the animation first.'),
          duration: Duration(seconds: 3),
          backgroundColor: Colors.red,
        ),
      );
